...
const assignmentsSlice = createSlice({
    ...
    initialState,
    reducers: {
        initializeAssignments: (state) => {
            if (state.assignments.length === 0) {
                state.assignments = assignments as Assignment[];
            }
        },
        addAssignment: (state, { payload: assignment }) => {
            ...
            state.assignments = [newAssignement, ...state.assignments] as any;
...
export const { addAssignment, deleteAssignment, updateAssignment,initializeAssignments} =
